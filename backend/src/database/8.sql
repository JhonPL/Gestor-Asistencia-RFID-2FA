...
-- PostgreSQL — Script completo v8 · Variante GOOGLE OAuth 2.0
...
--   ✎ estado_verificacion: nuevos estados más descriptivos
--       sin_app    → El estudiante no tiene la app instalada/registrada
--       registrado → El estudiante se logueó en la app
--       pendiente  → Pasó tarjeta RFID, falta confirmar en la app
--       verificado → Completó verificación biométrica + GPS exitosa
--       rechazado  → Verificación fallida (biometría o ubicación inválida)
--   ✎ dispositivo_movil: constraint UNIQUE por persona_id (un registro por estudiante)
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.estado_verificacion (
    ...
    CONSTRAINT chk_estado_verificacion_nombre CHECK (nombre IN (
        'sin_app',    -- Por defecto: estudiante sin app instalada/registrada
        'registrado', -- Estudiante se logueó en la app
        'pendiente',  -- Pasó tarjeta RFID, falta confirmar en la app
        'verificado', -- Completó verificación biométrica + GPS exitosa
        'rechazado'   -- Verificación fallida (biometría o ubicación inválida)
    ))
...
COMMENT ON TABLE public.estado_verificacion IS
    'sin_app=sin app instalada | registrado=tiene app | pendiente=pasó tarjeta, falta confirmar | verificado=2FA OK | rechazado=2FA falló';

INSERT INTO public.estado_verificacion (nombre) VALUES
    ('sin_app'), ('registrado'), ('pendiente'), ('verificado'), ('rechazado')
ON CONFLICT DO NOTHING;
...
COMMENT ON COLUMN public.sesion_clase.persona_id IS 'Docente asignado al curso.';
...
COMMENT ON COLUMN public.asistencia.estado_verificacion_id IS 'sin_app | registrado | pendiente | verificado | rechazado';
...
CREATE TABLE IF NOT EXISTS public.dispositivo_movil (
    ...
    CONSTRAINT dispositivo_movil_persona_key    UNIQUE (persona_id),
...
COMMENT ON COLUMN public.dispositivo_movil.persona_id    IS 'Un único dispositivo activo por persona.';
COMMENT ON COLUMN public.dispositivo_movil.plataforma    IS 'ios | android';
COMMENT ON CONSTRAINT dispositivo_movil_persona_key ON public.dispositivo_movil IS
    'Garantiza un solo registro por persona. ON CONFLICT (persona_id) DO UPDATE actualiza el dispositivo.';
