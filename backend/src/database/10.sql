...
-- PostgreSQL — Script completo v10 · Variante GOOGLE OAuth 2.0
-- Universidad Cooperativa de Colombia · Villavicencio, Meta
-- ============================================================
-- Cambios v10 respecto a v8:
--   * Añade dispositivo_movil.installation_id para vincular una
--     instalación móvil única por estudiante.
--   * Añade un índice único parcial para impedir que una instalación
--     quede vinculada a más de una persona.
--   * Mantiene la restricción de un dispositivo por persona.
--
-- Este archivo es autocontenido para crear una base nueva desde cero.
-- No modifica ni reemplaza los scripts de versiones anteriores.
-- ============================================================
...
COMMENT ON COLUMN public.dispositivo_movil.persona_id IS
    'Un único dispositivo activo por persona.';
COMMENT ON COLUMN public.dispositivo_movil.installation_id IS
    'Identificador aleatorio persistente de la instalación móvil autorizada.';
COMMENT ON CONSTRAINT dispositivo_movil_persona_key ON public.dispositivo_movil IS
    'Garantiza un solo registro por persona.';

CREATE UNIQUE INDEX IF NOT EXISTS dispositivo_movil_installation_key
    ON public.dispositivo_movil (installation_id)
    WHERE installation_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_dispositivo_persona
    ON public.dispositivo_movil (persona_id);
...
CREATE INDEX IF NOT EXISTS idx_verif_asistencia
    ON public.verificacion_biometrica (asistencia_id);
CREATE INDEX IF NOT EXISTS idx_verif_dispositivo
    ON public.verificacion_biometrica (dispositivo_movil_id);
